---
name: This is a fork of last_narrow_ablation.asm test designed towards testing correctness.
description: >
    To test for correctness of the final implementation we check parametrization that was deemed redundant
    for researching the issue.
dumped_symbols:
  times: auto
  cyc_in_it: auto
  flags: auto
...
product:
    - code: [flash ]
      ...
      lsu_addressing:
        ...
        - "[{base_reg}, 2]"
        - "[{base_reg}, r4]"
        - "[{base_reg}], 4" # Writeback, but reads the same base address
        - "[{base_reg}, 2]!"
      base_reg:
        - r7
        - sp
...
{% block code %}
.equ dwt_aligned, dwt_base_sym + 4
...
{% for b_in_it, bcond in itertools.product((True, False), ('', cond, notcond,))
   if (bcond or not b_in_it) and (b_in_it or ('b.' in b_instr or (b_instr == 'nop.n' and not bcond))) and (bcond == notcond or 'nop' in b_instr) %}

@ same for  'ldr.w sp, [r4]', 'ldr.w r5, [r5, r4]', 'adds.w r5, r5'
{% for dummy_instr in ('ldr.w r6, [r1]', 'add.w r5, r5', 'mla.w r5, r4, r6, r5', 'umull.w r5, r4, r5, r4', 'nop.w') %}
{% for x_cycles2 in (range(7, 11) if not lbEn else (range(1, 5) if x_cycles1 == 0 else range(5, 9))) %}
{% for x3_cycles in range(1, 6) %}
  ...
  {% set it_instr -%}
  i{{'t' if first_it else ''}}{{'e' if first_it == 'e' else 't'}}{{ {True: 't', False: 'e'}[bcond == it_cond] if b_in_it else ''}}.n {{it_cond}}
  {% endset %}
  ...
  ldr{{cond}}.w r6, {{lsu_addressing.format(base_reg=base_reg)}}
...
save_times_and_flags:
  mrs.w r1, apsr
  sub.w r11, r3, r2
  sub.w r2, r6, {{ r2 if lsu_from == 'dwt' else r4 }}
  {{saveValue("times", r11, r3, r4)}}
  {{saveValue("flags", r1, r3, r4)}}
  {% if '2]' not in lsu_addressing or lsu_from != 'dwt' %}
  @ Unaligned reads from the counters return trash
