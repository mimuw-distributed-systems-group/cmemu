---
name: Test the tainted last narrow instruction pipelining.
description: >
    The tainted instruction is one 5 halfwords past it.
    It sometimes doesn't pipeline
...
{% for last_pad in ('', 'add.w r4, r4') %}
  ...
  add.w r3, r3
  {% if cond %}
  i{% if it_len >= 1 %}t{% endif %}{% if it_len >= 2 %}t{% endif %}{% if it_len >= 3 %}{{cond_2}}{% endif %}{% if it_len >= 4 %}t{% endif %}.n {{cond}}
  {% else %}
  nop.n
  {% endif %}
  {% call itify(it_len >= 1 and cond) %}add.w r3, r3{% endcall %}
  {% call itify(it_len >= 2 and cond) %}{{ first.format(**format_args1) }}{% endcall %}
  {% call itify(cond_2|replace('e', 'ne')|replace('t', 'eq')) %}{{ second.format(s='s' if not cond_2 else '', **format_args2) }}{% endcall %}
  {% call itify(it_len >= 4 and last_pad and cond ) %}{{ last_pad }}{% endcall %}
  {% call itify(it_len >= 4 and not last_pad and extra and cond ) %}{{ extra }}{% endcall %}

  @ Get finish time
1: @ see https://ftp.gnu.org/old-gnu/Manuals/gas-2.9.1/html_chapter/as_5.html#SEC48
  {% call itify(it_len >= 4 and not last_pad and not extra and cond ) %}ldr.w  r3, [r0]{% endcall %}
