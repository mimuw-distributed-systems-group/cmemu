...
description: >
    This is an elaboration around code such as:
        ...
        itt.n	mi              @ <- another it, theoretically could fold (doesn't!)
...
{% block code %}
    {% for counter, save_func in [(CYCCNT, "save_times_and_flags"), (LSUCNT, "save_lsucnt"), (FOLDCNT, "save_foldcnt")] %}
        ldr.w r0, dwt
        add.w r0, {{counter}}
...
{% for cond_2 in ('t', 'e', '') if (not cond_2) == (it_len < 3) or (second.startswith('b') and it_len < 3)  %}
  ...
  {% call itify(it_len >= 4  and notcond ) %}{{ last_pad.format(s='', **format_args2) }}{% endcall %}
  {{extra}}
  itt.n {{notcond}}
