---
name: Test how lsr/str rX, [rA, rB] (register offset) behaves witn conflicts on read/write ports
description: >
    Every half-aligned address points to the magic 0x2000_2000 in SRAM

    T1 encoding allows any 3 low registers.
    Note, that for LDR (register): LDR<c>.W <Rt>, [<Rn>, <Rm>, LSL #<imm2>]
    - Rn/Rm cannot by PC
    - Rm cannot be SP, but Rn may be SP
    - Rt may be PC/SP, but restrictions in IT block apply (and there is a weird check of aligned addr)

    In STR(register), PC is not allowed anywhere.

    TODO: symbols may refer to sram..., but actually reside in GPRAM
dumped_symbols:
  counter: auto
  #rough_foldcnt: auto
  rough_lsucnt: auto
...
#      stall2: [x_cyc]
      ...
      prev_instr: [
        ...
        "add.n {dest}, {dest}, {reg_0_or_2}",
        "add.w {dest}, {dest}, {reg_0_or_2}",
        ...
        # Updating flags
        "adds.w {dest}, {dest}, {reg_0_or_2}", # Only wide in IT
        # branches
        # invalid in it block :(
...
@  {% endif %}
  ...
  i{{ t.strip() }}.n eq
  {% endif %}
  {% call itify(t[0]) %}{{ prev_instr.format(tested_label=tested_label, jump_label=jump_label, addr_reg=addr_reg, dest_no_sp=dest, dest=dest, reg_0_or_2=r4) }}{% endcall %}
{{ tested_label }}:
  @ TODO: vary Rt and Rn independently
  {% call itify(t[1]) %}{{ tested_instr.format(Rt=Rt, Rn=Rn, Rm=Rm) }}{% endcall %}

  {% call itify(t[2]) %}{{ pad1.format(addr_reg=addr_reg, jump_label=jump_label, dest=dest) }}{% endcall %}
  {% call itify(t[3]) %}{{ pad2.format(addr_reg=addr_reg, jump_label=jump_label, dest=dest) }}{% endcall %}
