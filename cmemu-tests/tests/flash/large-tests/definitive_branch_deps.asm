---
name: Branch dependencies seem to have some misemulations
description: |
    The good-old deps of:

       ldr lr, [...]
       <pipelined>
       bx lr

    was problematic and led to our discovery of functional bug in the processor:

      umull x, lr, y, z
      mla   w, v, lr
      bx    lr

    Later, we found that add PC, x has a secret AGU-dependency on R0.

    However, Fred found some new cases involving:
      - add PC, SP
      -
        ldr lr, [...]
        ldr pc, [...]
    ...
    Here we aim to generate all of these.

    Test derived from phantom_it_curse, branch_target, and b_speculative
...
#  - 'cmn.w {out_reg}, #42'
##    # Not updating flags
#  - mov.n {out_reg}, {out_reg}
##  - mov.w {out_reg}, {out_reg}
##  - add.w {out_reg}, r4
#  - add.n {out_reg}, r4
##    # TODO: ADD MULL/MLA INSTRUCTION
##    # Should be no-op (r4 == 0)
##  - umlal.w {out_reg}, r7, {out_reg}, r4
#  - mla.w {out_reg}, r7, r4, {out_reg}
##    # Updating flags
#  ##- adds.w {out_reg}, r4, {out_reg} # illegal for sp
#  - adds.w {out_reg}, {out_reg}, r4 # Only wide in IT
##    # LSU - out_reg as target and out_reg as writeback, @-comment is parsed for the memory location
  - ldr.w {out_reg}, {in_label} @ Flash
  - ldr.w {out_reg}, [{addr_reg}] @ Flash
  - ldr.w {out_reg}, [{addr_reg}] @ SRAM
#  - ldr.n {out_reg}, [{addr_reg}] @ Flash
  - ldr.w {out_reg}, [{addr_reg}, r4] @ Flash
  - ldr.w {out_reg}, [{addr_reg}, r4] @ SRAM
# Note: ony pre-indexed so we can manipulate the address no matter if we need the value of the address itself
# TODO: different addr/out regs (must be in writeback)
#  - ldr.w {out_reg}, [{addr_reg}, 4]! @ Flash
  - ldr.w r7, [{addr_reg}, 4]! @ Flash
#  - ldr.w r7, [{addr_reg}, r4]
# These may share addr_reg with base_reg, so make the scenario load from SRAM instead of Flash in some cases
#  - str.w {str_src_reg}, [{addr_reg}] @ SRAM
  - str.w {str_src_reg}, [{addr_reg}, r4] @ SRAM
  - str.w {str_src_reg}, [{addr_reg}, 4]! @ SRAM

  # Offsets are handled at the base register level, so we're skipping SP here
  # Note: non-zero offsets were not tested with branch_slot_loc == destination
  prev_offset: # NOTE: if enabling non-zero offset, don't enable non-lsu prev_instr-s
  - 0
#  - 1
#  - 2
  scenario_offset: # NOTE: if enabling non-zero offset, don't enable non-ldr scenarios
  - 0
#  - 1
  - 2
...
#  - pop.n {{pc}}
#  - pop.n {{{some_reg}, pc}}
#  - pop.w {{pc}} # special encoding
#  - pop.w {{{some_reg}, pc}}
# Note: ony post-indexed so we the address is known no matter if prev manipulates it
#  - ldr.w pc, [{base_reg}] @ Flash
#  - ldr.w pc, [{base_reg}, {dep_reg}] @ Flash
  - ldr.w pc, [{base_reg}], 4 @ Flash
#  - ldm.w {base_reg}, {{{some_reg}, pc}} @ Flash
#  - ldm.w {base_reg}!, {{{some_reg}, pc}} @ Flash
#  - ldr.w pc, [{base_reg}] @ SRAM
#  - ldr.w pc, [{base_reg}, {dep_reg}] @ SRAM
#  - ldr.w pc, [{base_reg}], 4 @ SRAM
#  - ldm.w {base_reg}, {{{some_reg}, pc}} @ SRAM
#  - ldm.w {base_reg}!, {{{some_reg}, pc}} @ SRAM
  # Indirect - register
#  - add.n pc, {dest_reg}
#  - mov.n pc, {dest_reg}
...
#  - blx.n {dest_reg}
  # Parameter-less
# TODO: these are fall-through!
#  - isb.w
#  - mov.n pc, pc; bx lr # Skips 2 bytes!
  # Various
#  - negative
#  - mispredicted
#  - speculative
#  - svc # not implemented yet
  ...
  # Also, look for decode-time interaction
#  - nop.n
#  - nop.w
#  - add.w {next_reg}, r2
#  - add.n {next_reg}, r2
  ...
  - add.n pc, {next_reg}
  - isb.w
  - ldr.w pc, ={wrong_label}+1
# TODO: add shift to support non-writeback ldrs
  - ldr.w pc, [{next_reg}]
#  - b.n {wrong_label}
#  - b.w {wrong_label}
#  - beq.n {wrong_label}
...
{% pyset next_reg_can_share_base_reg =
    ...
    writeback_shift('base_reg', scenario) == 4 and
...
{% pyset next_reg_can_share_addr_reg =
    instr_stem(prev_instr) != 'str' and
    (needs_format('out_reg', prev_instr) or 'SRAM' not in prev_instr)
%}
    ...
    {{bad_target_mem_loc}}:
        ...
        {% elif needs_format('next_reg', branch_slot) and
            ...
            (next_reg != base_reg or next_reg_can_share_base_reg and instr_stem(branch_slot) == 'ldr')
             -%} @ Value?
            {% if instr_stem(branch_slot) == 'ldr' and base_reg != next_reg %}
