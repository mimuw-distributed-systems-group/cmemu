...
{% for offset_cnt in range(offsetRange) %}
    {% set offset = offset_cnt * 8 %} @ * 2(words) * 4(bytes)
    @ Prepare value to store
    ldr.w  r4, ={{values[0]}}
    ldr.w  r5, ={{values[1]}}
    @ Prepare address to store to
    ldr.w  r6, =rep_{{offset_cnt}}_memory
...
{% for offset_cnt in range(offsetRange) %}
    ...
    @ There are 2 words for each possible offset value.
    {% for _ in range(offsetRange*2) %}
        .word 0
    {% endfor %}
{% endfor %}
